    ...
    if (point.canvasy === null || point.canvasy != point.canvasy) {
      ...
      if (prevCanvasX !== null) {
        if (strokeWidth) {
          ...
          {
			// JRH 2018-08-02
			// Suppress sub-pixel structure leading to graphics glitches
			// for data with quantization noise.  We only want to draw
			// lines that move a significant distance (more than a few
			// pixels in x or y), as there seems to be overshooting
			// problems with short line segments.  Note that I am 
			// computing a distance proxy (dx + dy) for speed reasons.
			// The (dy > dx) check is looking for lines with large
			// slopes.
			var dy = Math.abs(point.canvasy - prevCanvasY);
			var dx = point.canvasx - prevCanvasX;
            if (((dx + dy) < 5) && (dy > dx))
              continue;
